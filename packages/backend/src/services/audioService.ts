...
export function getCurrentAudioOutput(): { card: string; name: string } | null {
  try {
...
export function configureAudioOutput(params: {
  card: string;
  name: string;
...
export function restartMPD(): { success: boolean; warning?: string } {
...
export function stopMPD(): { success: boolean; warning?: string } {
  if (checkTool("systemctl")) {
    try {
...
export function setOutputMode(mode: OutputMode): {
  ...
  if (mode === "browser") {
    ...
    try {
      ...
      return { success: false, warning: `Cannot write to ${MPD_CONFIG_PATH}.` };
    }
  } else {
...
export function getOutputDeviceName(): string | null {
