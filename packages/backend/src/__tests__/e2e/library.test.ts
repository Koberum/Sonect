...
describe("Library e2e (supertest + real seeded DB)", () => {
  let close: () => void;
  ...
  it("GET /library/artists returns seeded artists", async () => {
    track.create({ artist: "E2E Artist One" });
    track.create({ artist: "E2E Artist Two" });

    const res = await request(app).get("/library/artists");
    expect(res.status).to.equal(200);
    expect(res.body.items).to.be.an("array");
    expect(res.body.total).to.be.at.least(2);
  });

  it("GET /library/search?q= finds seeded tracks", async () => {
  ...
  it("GET /library/search?q= empty returns empty arrays", async () => {
    const res = await request(app).get("/library/search").query({ q: "" });
    expect(res.status).to.equal(200);
  ...
  it("GET /library/tracks returns paginated seeded tracks", async () => {
    track.createMany(3);
    const res = await request(app).get("/library/tracks").query({ limit: 2 });
    expect(res.status).to.equal(200);
